      ...
      if (!finished)
      {
        ...
        for(int num=0;num<collatzData.size();num++) //1000
        {
          ...
          for(int i=0;i<currentMaxSteps;i++)
          {
            ...
            if (steps>currentMaxSteps-(currentMaxSteps/6.0) )
            {
              b+=10;
              g+=30;
              r-=10;
...
void showText()
{
  ...
  fill(0);
  noStroke();
  rect(width-50,height-30,width,height);
  fill(255);
  text(textToShow,width-50,height-15);
  textAlign(CENTER);
}
